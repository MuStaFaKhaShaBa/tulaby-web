...
import LectureTableRecord from "./LectureTableRecord/LectureTableRecord";
...
export default function EventTable({
  ...
  return (
    <div className="table-responsive shadow-sm px-3 py-4 position-relative">
      {isLoading ? (
        <LoadingComponent />
      ) : isError ? (
        <div className="p-4">
        ...
        <>
          ...
          <table className="table text-nowrap mb-0 text-center align-middle">
            <thead className="text-dark fs-4">
              <tr>
                ...
                <th className="border-bottom-0">
                  <h6 className="fw-semibold mb-0">{t(`persons.${isLecture?"professor":"instructor"}`)}</h6>
                </th>
                <th className="border-bottom-0">
            ...
            <tbody className="list-items">
              ...
              {data?.items.map((record) => (
                <LectureTableRecord key={record.code} lang={lang} {...record} />
              ))}
              {/* <!-- Records --> */}
            </tbody>
          </table>
        ...
        <div className="p-4">
          <div className="alert bg-transparent alert-info mb-0">
            <p className="mb-0">{t(`errors.${isLecture?"Lectures":"Sessions"}Records`)}</p>
          </div>
        </div>
      )}
    </div>
